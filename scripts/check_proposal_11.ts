...
function checkProposal11Logs(
  ...
  // second event should be a NewResolver for subdomain
  const EXPECTED_NEWRESOLVER_EVENT = {
    node: LICENSE_GRANTS_NAME_NODE,
    resolver: ENS_PUBLIC_RESOLVER_2_ADDRESS,
  };
  validateEventArgsArray.push({
    expectedEvent: EXPECTED_NEWRESOLVER_EVENT,
    expectedOrigin: ENS_REGISTRY_WITH_FALLBACK_ADDRESS,
  ...
  const callData = ensRegistryContract.interface.encodeFunctionData(
    "setSubnodeRecord",
    [
      UNISWAP_NAME_NODE,
  ...
  const txHashValues = [
    ...
    0, // value = 0
    "", // empty signature
    callData, // calldata to call ENS registry
  ...
  // fourth event should be TextChanged from the PublicResolver
  ...
  // fifth event should be ExecuteTransaction from UNI timelock
  ...
  // sixth and last event should be ProposalExecuted from UNI governor
  const EXPECTED_PROPOSAL_EXECUTED_EVENT: { [key: string]: any } = {
    id: ethers.BigNumber.from(11),
  };
  validateEventArgsArray.push({
